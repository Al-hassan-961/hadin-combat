...
- 🧠 **Silent global-AI coach** *(optional)* — auto-activates in the background to give a second opinion on every frame (strike type, confidence, form score, short form/tactical cues, fatigue). Uses Google's **official free-tier** Multimodal Live API when a `GEMINI_API_KEY` is set; otherwise it silently falls back to the local ML coach. No popups, no interaction, and the provider is **never shown in the UI** — it just improves the coaching tips. See `app/gemini_free.py` and `scripts/prepare_ai_coach.sh`.
